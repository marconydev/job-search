 ...
 */
function interpretarUrlWorkable(
  url: string
): DadosUrlWorkable | null {
  try {
    const urlAnalisada =
      new URL(url)
...
function encontrarVaga(
  ...
  const shortcode =
    lerTexto(valor.shortcode)

  if (
    shortcode?.toLowerCase() ===
    codigoVaga.toLowerCase()
  ) {
    return valor
  }

  const shortlink =
    lerTexto(valor.shortlink)
...
function extrairNomeEmpresa(
  ...
  return (
    lerTexto(dados.name) ??
    lerTexto(dados.company_name) ??
    lerTexto(dados.companyName)
  )
}
...
function detectarRemoto(
  ...
  if (ehObjeto(localizacao)) {
    if (
      localizacao.telecommuting === true
    ) {
      return true
    }
 ...
 */
export async function extrairVagaWorkable(
  url: string
): Promise<VagaExtraida | null> {
  const dadosUrl =
    interpretarUrlWorkable(url)
  ...
  const urlApi =
    `${URL_API_PUBLICA}/` +
    `${encodeURIComponent(dadosUrl.conta)}` +
    "?details=true"
