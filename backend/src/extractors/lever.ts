 ...
 */
function converterHtmlParaTexto(html: string | undefined): string | null {
  if (!html) {
    return null
  }
 ...
 */
function interpretarUrlLever(url: string): DadosUrlLever | null {
  try {
    const urlAnalisada = new URL(url)
    const hostname = urlAnalisada.hostname.toLowerCase()

    const ehGlobal = hostname === "jobs.lever.co"

    const ehEuropa = hostname === "jobs.eu.lever.co"
    ...
    const dominioApi = ehEuropa ? "https://api.eu.lever.co" : "https://api.lever.co"
 ...
 */
function montarListas(listas: RespostaLever["lists"]) {
  if (!listas?.length) {
    return null
  }
  ...
  const secoes = listas
    .map(lista => {
      const titulo = normalizarTexto(lista.text)

      const conteudo = converterHtmlParaTexto(lista.content)
 ...
 */
function montarDescricao(vaga: RespostaLever): string | null {
  const descricao = normalizarTexto(vaga.descriptionPlain) ?? normalizarTexto(vaga.openingPlain)

  const listas = montarListas(vaga.lists)

  const adicional = normalizarTexto(vaga.additionalPlain)

  const partes = [descricao, listas, adicional].filter((parte): parte is string => Boolean(parte))
 ...
 */
function detectarRemoto(vaga: RespostaLever) {
  if (vaga.workplaceType === "remote") {
    return true
  }

  if (vaga.workplaceType === "on-site" || vaga.workplaceType === "hybrid") {
    return false
  }

  const texto = [vaga.text, vaga.categories?.location, vaga.descriptionPlain]
 ...
 */
export async function extrairVagaLever(url: string): Promise<VagaExtraida | null> {
  const dadosUrl = interpretarUrlLever(url)
  ...
  try {
    const resposta = await fetch(dadosUrl.urlApi, {
      headers: {
    ...
    const dados = (await resposta.json()) as RespostaLever

    const titulo = normalizarTexto(dados.text)
    ...
    const localizacao =
      normalizarTexto(dados.categories?.location) ??
      (dados.categories?.allLocations?.length ? dados.categories.allLocations.join(", ") : null) ??
      normalizarTexto(dados.country ?? undefined)
    ...
    return {
      ...
      tipoContratacao: normalizarTexto(dados.categories?.commitment),

      dataPublicacao: null,
      validaAte: null,

      remoto: detectarRemoto(dados),

      urlCandidatura: normalizarTexto(dados.applyUrl) ?? normalizarTexto(dados.hostedUrl) ?? url
    }
  } catch {
    return null
  }
